...
console.log = function () {
  log.apply(console, [parseTimestampToHumanDate(Date.now())].concat(arguments[0]), logToFile(arguments[0]));
};
...
const attemptCounts = Array.from(Array(Number(configManager.getConfig().ATTEMPT_WAIT_PAYMENT_COUNTS)).keys());
    ...
    try {
      ...
      users.forEach(async (user) => {
        if (
          ...
          try {
            console.log("Autopayment started for: @" + user.nickname + " time_sub: " + user.time_sub);
            ...
            const payment = await yooKassa.createPayment(
              {
                ...
                receipt: {
                  ...
                  items: [
                    {
                      description: `Автоплатеж по подписке, ${configManager.getConfig().SUB_PRICE} рублей`,
                      quantity: "1.00",
                      amount: {
                        value: configManager.getConfig().SUB_PRICE,
                        currency: "RUB",
            ...
            await bot.sendMessage(
              targetChatId,
              `Автоплатеж по подписке инициирован (${configManager.getConfig().SUB_PRICE} рублей)`,
            );
            let status;
            let savedPayment;

            console.log(`Autopayment attempt ${user.user_id} (@${user.nickname})`);
            ...
            if (status === "succeed" || status === "waitingForCapture") {
              if (status === "succeed")
                await bot.sendMessage(targetChatId, "Автоплатеж по подписке прошел успешно. Спасибо!");
              ...
              const date = new Date();
              const timeSub = date.setDate(date.getDate() + Number(configManager.getConfig().SUB_PERIOD_DAYS));

              const paymentId = savedPayment.id;
              const paymentMethod = savedPayment.payment_method.id;
              const cardNumbers =
                savedPayment.payment_method.card.first6 + "," + savedPayment.payment_method.card.last4;
...
const checkIsAdmin = (username) => {
  return configManager.getConfig().ADMIN_TG_ACCOUNT_USERNAMES.split(",").includes(username);
};
...
const initBot = function () {
  ...
  bot.on("text", async (msg) => {
    try {
      ...
      if (msg.text === "Подписка") {
        ...
        const subscriptionStatus = timeSub ? (await db.getSubscriptionStatus(msg.from.id)).payment_status : undefined;
        ...
        if (subscriptionStatus === "succeeded") {
          ...
          const payment = await yooKassa.createPayment(
            {
              ...
              receipt: {
                ...
                items: [
                  {
                    description: `Оформление подписки, ${configManager.getConfig().SUB_PRICE} рублей`,
                    quantity: "1.00",
                    amount: {
                      value: configManager.getConfig().SUB_PRICE,
                      currency: "RUB",
          ...
          await bot.sendMessage(
            msg.chat.id,
            `Стоимость подписки на ${configManager.getConfig().SUB_PERIOD_DAYS} дней составляет ${
              configManager.getConfig().SUB_PRICE
          ...
          for (const attempt of attemptCounts) {
            console.count(`Waiting payment from user ${msg.from.id} (@${msg.from.username})`);
          ...
          if (status === "succeed" || status === "waitingForCapture") {
            ...
            const getCardSavedStatus = isCardSaved ? "Автоплатежи активны" : "Автоплатежи не активны";
            ...
            if (status === "succeed") {
              ...
              await bot.sendMessage(msg.chat.id, "Введите, пожалуйста, своё ФИО (через пробел)");
              ...
              await bot.sendMessage(msg.chat.id, `Введите, пожалуйста, свой email`);
            ...
            const paymentId = savedPayment.id;
            const paymentMethod = savedPayment.payment_method.id;
            const cardNumbers = savedPayment.payment_method.card.first6 + "," + savedPayment.payment_method.card.last4;

            const userNickname = await db.getUserNickname(userId);

            if (!userNickname) await db.addNewUser(userId, msg.from.username, fullname);
      ...
      if (msg.text === "Прервать подписку") {
        const subscriptionStatusSource = await db.getSubscriptionStatus(msg.from.id);
        ...
        if (subscriptionStatus === "succeeded") {
          await db.setSubscription(userId, 0, 0, 0, 0, "cancelledByUser", new Date().toISOString(), 0);
          ...
          for (let chat of adminChats) {
            await bot.sendMessage(chat.chat_id, `Пользователь @${username} (id ${userId}) отменил подписку`);
          }
        } else {
          if (subscriptionStatus === "cancelledByUser")
            await bot.sendMessage(
              msg.chat.id,
              `Невозможно отменить подписку, подписка была отменена: ${parseTimestampToHumanDate(subscriptionDate)}`,
            );
          else await bot.sendMessage(msg.chat.id, `Невозможно отменить подписку, подписка не активна`);
        }
      }
      ...
      // Admin commands
      if (msg.text === "Экспортировать платежную историю всех пользователей" && checkIsAdmin(username)) {
      ...
      if (msg.text === "Экспортировать статус подписок всех пользователей" && checkIsAdmin(username)) {
      ...
      if (msg.text === "Текущий период оплаты" && checkIsAdmin(username)) {
        await bot.sendMessage(msg.chat.id, configManager.getConfig().SUB_PERIOD_DAYS);
      }
      if (msg.text === "Изменить стоимость подписки" && checkIsAdmin(username)) {
        await bot.sendMessage(msg.chat.id, "Введите новую стоимость подписки, к примеру: 100.00");
        ...
        await bot.once("text", async (reply) => {
          ...
          await bot.sendMessage(msg.chat.id, `Стоимость подписки установлена: ${userReply}`);

          const usersChatsSource = await db.getAllUsersChats();

          const allChats = usersChatsSource.map((item) => item?.chat_id);
          for (const chat of allChats) {
            await bot.sendMessage(chat, `Внимание! Стоимость подписки изменена и равна: ${userReply} рублей`);
          }
        });
      }

      if (msg.text === "Изменить период оплаты" && checkIsAdmin(username)) {
        await bot.sendMessage(msg.chat.id, "Введите новое значение периода оплаты, к примеру: 30.44");
        ...
        await bot.once("text", async (reply) => {
          ...
          console.log(`ADMIN (${username}): SUB_PERIOD_DAYS changed to ${userReply}`);

          await bot.sendMessage(msg.chat.id, `Период оплаты изменен: ${userReply} дней`);
        });
      }

      if (msg.text === "Экспортировать пользователей с неоплаченной подпиской" && checkIsAdmin(username)) {
        ...
        await sendHistoryFile(
          historySourceWithFullname,
          configManager.getConfig().OUTPUT_FOLDER,
          "bad-subscription-status-users-subscriptions-report-" + Date.now() + ".pdf",
          bot,
          msg.chat.id,
          "Список пуст",
        );
  ...
  bot.on("polling_error", (err) => console.error("polling_error: " + err?.data?.error?.message));
};
